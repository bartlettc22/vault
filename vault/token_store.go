...
var (
	...
	destroyCubbyhole = func(ctx context.Context, ts *TokenStore, tokenID string) error {
		...
		if tokenMapping != nil {
			err := ts.cubbyholeBackend.revoke(ctx, salt.SaltID(ts.cubbyholeBackend.saltUUID, tokenMapping.ID, salt.SHA256Hash))
			if err != nil {
				return err
			}
		}
...
func NewTokenStore(ctx context.Context, c *Core, config *logical.BackendConfig) (*TokenStore, error) {
	...
	if err != nil {
		fmt.Printf("failed to create memdb for token store: %v", err)
...
func (ts *TokenStore) storeCommon(ctx context.Context, entry *TokenEntry, persist bool) error {
	tokenMapping, err := ts.CreateTokenMapping(entry, persist)
	if err != nil {
		return err
	}
